pipeline {
  ...
  parameters {
    string(name: 'VNF_NAMES', description: 'Names of VNFs to be terminated separated by commas, e.g. vnf-1, vnf-2, vnf-3', defaultValue: "NTAS2")
  }
  environment {
    ...
    ARTIFACTORY_URL = "https://artifactory.eficode.io"
    ARTIFACTORY_PATH = "Nokia/NTAS"
    // TODO: Can these be read from VNFD?
    NTAS_IP = "1.1.1.1"
    NTAS_CONF_IP = "1.1.1.1"
  }
  stages {
    stage('Check prerequisites') {
      steps {
        ...
        git credentialsId: 'ntasbot', url: 'https://git.eficode.io/scm/entas/pipelines.git'
        sh 'pip install -r requirements.txt'
        script {
      ...
      steps {
        script {
          ...
          if (diffCode != 0) {
            ...
            sh 'git -c user.name="Bot" -c user.email="test@example.com" commit -m "Add files from artifactory - ${VERSION_PATH}"'
            sh "git push https://${NTAS_BOT_USR}:${NTAS_BOT_PSW}@git.eficode.io/scm/entas/pipelines.git"
          } else {
