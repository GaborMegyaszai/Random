...
cbamlib.modify_vnf(vnf["id"], f"{download_directory}/extensions_helpalab3_tastest2.json")

# Upload image to CBIS
oslib = OpenStackLibrary()
oslib.connect(cloud="elisa")
...
#image_uri = f"https://{os.environ['NTAS_BOT_USR']}:{os.environ['NTAS_BOT_PSW']}@artifactory.elisa.eficode.io/Nokia_TAS/cirros-0.4.0-x86_64-disk.img"
...
modified = ""
instantiation_json = open(f"{download_directory}/instantiation_data_helpalab3_tastest2_no_zone.json", "r+")
...
cbamlib.instantiate_vnf(vnf["id"], f"{download_directory}/instantiation_data_helpalab3_tastest2_no_zone.json")
